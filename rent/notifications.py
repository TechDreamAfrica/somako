...
def notify_rental_confirmed(booking):
    ...
    send_notification(
        ...
        message=f'Your equipment booking for {item_name} has been confirmed! Dates: {booking.start_date.strftime("%b %d")} - {booking.end_date.strftime("%b %d, %Y")}. Total: GHS {booking.total_price}',
        channels=['in_app', 'sms', 'whatsapp'],
        reference_id=str(booking.id),
        reference_type='RentalBooking',
        data={
...
def notify_rental_started(booking):
    ...
    send_notification(
        ...
        message=f'Your equipment rental for {item_name} has started. Enjoy until {booking.end_date.strftime("%b %d, %Y")}!',
        channels=['in_app', 'sms'],
        reference_id=str(booking.id),
        reference_type='RentalBooking'
    )
...
def notify_rental_ended(booking):
    ...
    send_notification(
        ...
        title='Rental Ended',
        message=f'Your equipment rental for {item_name} has ended. Thank you for using Soma Ko!',
        channels=['in_app', 'sms', 'whatsapp'],
        reference_id=str(booking.id),
        reference_type='RentalBooking'
    )
...
def notify_rental_cancelled(booking, cancelled_by):
    ...
    if cancelled_by == booking.customer:
        ...
        send_notification(
            ...
            notification_type='rental_cancelled',
            title='Booking Cancelled',
            message=f'Your booking for {item_name} has been cancelled by the owner.',
            channels=['in_app', 'sms', 'whatsapp'],
            reference_id=str(booking.id),
            reference_type='RentalBooking'
        )
