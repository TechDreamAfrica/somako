"""
Pharmacy PWA URLs - Standalone Progressive Web App routes
"""

from django.urls import path
from . import views

app_name = 'pharmacy_pwa'

urlpatterns = [
    # PWA Dashboard
    path('', views.pwa_dashboard, name='dashboard'),

    # Browse Pharmacies
    path('pharmacies/', views.pwa_pharmacy_list, name='pharmacy_list'),
    path('pharmacies/<int:pk>/', views.pwa_pharmacy_detail, name='pharmacy_detail'),

    # Browse Medicines
    path('medicines/', views.pwa_medicine_list, name='medicine_list'),
    path('medicines/<int:pk>/', views.pwa_medicine_detail, name='medicine_detail'),
    path('categories/<str:category>/', views.pwa_category_medicines, name='category_medicines'),

    # Cart & Checkout
    path('cart/', views.pwa_cart_view, name='cart'),
    path('cart/add/<int:medicine_id>/', views.pwa_add_to_cart, name='add_to_cart'),
    path('cart/update/<int:cart_item_id>/', views.pwa_update_cart_item, name='update_cart_item'),
    path('cart/remove/<int:cart_item_id>/', views.pwa_remove_from_cart, name='remove_from_cart'),
    path('cart/clear/', views.pwa_clear_cart, name='clear_cart'),
    path('checkout/', views.pwa_checkout, name='checkout'),
    path('checkout/confirm/', views.pwa_confirm_order, name='confirm_order'),

    # Prescription Upload
    path('prescription/upload/', views.pwa_upload_prescription, name='upload_prescription'),
    path('prescription/scan/', views.pwa_scan_prescription, name='scan_prescription'),

    # Orders
    path('orders/', views.pwa_order_list, name='order_list'),
    path('orders/<str:order_number>/', views.pwa_order_detail, name='order_detail'),
    path('orders/<str:order_number>/track/', views.pwa_track_order, name='track_order'),
    path('orders/<str:order_number>/cancel/', views.pwa_cancel_order, name='cancel_order'),

    # Search & Filters
    path('search/', views.pwa_search, name='search'),

    # Pharmacy Owner Section
    path('manage/', views.pwa_owner_dashboard, name='owner_dashboard'),
    path('manage/orders/', views.pwa_manage_orders, name='manage_orders'),
    path('manage/orders/<int:order_id>/', views.pwa_order_detail_owner, name='order_detail_owner'),
    path('manage/orders/<int:order_id>/update-status/', views.pwa_update_order_status, name='update_order_status'),
    path('manage/medicines/', views.pwa_manage_medicines, name='manage_medicines'),
    path('manage/medicines/add/', views.pwa_add_medicine, name='add_medicine'),
    path('manage/medicines/<int:medicine_id>/edit/', views.pwa_edit_medicine, name='edit_medicine'),
    path('manage/medicines/<int:medicine_id>/toggle/', views.pwa_toggle_medicine, name='toggle_medicine'),
    path('manage/medicines/<int:medicine_id>/delete/', views.pwa_delete_medicine, name='delete_medicine'),
    path('manage/analytics/', views.pwa_analytics, name='analytics'),
    path('manage/settings/', views.pwa_pharmacy_settings, name='pharmacy_settings'),

    # Notifications
    path('notifications/', views.pwa_notifications, name='notifications'),
]
